<template>
  <div class="rounded-xl border border-white/10 bg-slate-900 p-4">
    ...
    <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      <div class="rounded-lg border border-white/5 bg-slate-950/60 p-3">
        <p class="text-xs text-slate-400">الإيرادات (صافي المبيعات)</p>
        <p class="mt-1 text-xl font-bold text-emerald-300">
          {{ formatMoney(financials.revenue, "locale") }}
        </p>
      </div>
      <div class="rounded-lg border border-white/5 bg-slate-950/60 p-3">
        <p class="text-xs text-slate-400">تكلفة البضاعة (COGS)</p>
        <p class="mt-1 text-xl font-bold text-rose-300">
          {{ formatMoney(financials.cogs, "locale") }}
        </p>
      </div>
      <div class="rounded-lg border border-white/5 bg-slate-950/60 p-3">
        <p class="text-xs text-slate-400">إجمالي الربح</p>
    ...
    <div class="mt-4 grid gap-2 text-sm text-slate-200 sm:grid-cols-2 lg:grid-cols-3">
      <p>
        مصروفات الفروع:
