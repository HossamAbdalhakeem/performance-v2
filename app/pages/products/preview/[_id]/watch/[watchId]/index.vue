...
const alias = route?.params?._id; // product alias from preview route
const watchId = route?.params?.watchId; // content _id from extra.contents_info

// ---- Product (same API as preview, resolved by alias) -------------------
const productRequestBody = {
  select:
    "type alias _id experience_id title summary about extra main_photo media fundraiser._id fundraiser.alias fundraiser.name ff",
